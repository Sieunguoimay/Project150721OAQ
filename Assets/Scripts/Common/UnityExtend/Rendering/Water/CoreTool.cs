...
namespace Common.UnityExtend.Rendering.Water
{
    ...
    public static class CoreTool
    {
        ...
        {
            var scaleOffset = Matrix4x4.TRS(new Vector3(0.5f, 0.5f, 0.5f), Quaternion.identity,
                new Vector3(0.5f, 0.5f, 0.5f));
            var scale = transform.lossyScale;
            var projMatrix = transform.localToWorldMatrix *
                             Matrix4x4.Scale(new Vector3(1.0f / scale.x, 1.0f / scale.y, 1.0f / scale.z));
            projMatrix = scaleOffset * cam.projectionMatrix * cam.worldToCameraMatrix * projMatrix;
            return projMatrix;
        }
