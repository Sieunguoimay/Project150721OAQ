...
namespace Gameplay.Visual.Piece.Activities
{
    #region Uncommon
    ...
    {
        private readonly CitizenAnimator _p;
        private readonly Func<Vector3> _target;

        private TransformControlTrack _jumping;
        private TransformControlTrack _facing;

        public ActivityJumpTimeline(CitizenAnimator p, Func<Vector3> target) : base((float) p.JumpTimeline.duration)
        {
            _p = p;
            _target = target;
        }
