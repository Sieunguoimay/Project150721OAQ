...
namespace texttools
{
    ...
    {
        [FormerlySerializedAs("textPath")] [SerializeField]
        ...
        [SerializeField] List<float> lengthList = new List<float>();

        [FormerlySerializedAs("sourceFont")] [SerializeField]
        TextToolsFont sourceFont = null;

        [FormerlySerializedAs("inputText")] [SerializeField] [TextArea(10, 10)]
        string inputText = "";

        [FormerlySerializedAs("fontSize")] [SerializeField]
        int fontSize = 72;

        [FormerlySerializedAs("characterSpace")] [SerializeField]
        float characterSpace = 0.0f;

        [FormerlySerializedAs("wordSpace")] [SerializeField]
        float wordSpace = 0.0f;

        [FormerlySerializedAs("lineSpace")] [SerializeField]
        float lineSpace = 0.0f;

        [FormerlySerializedAs("pathOffset")] [SerializeField]
        float pathOffset = 0.0f;

        [FormerlySerializedAs("textAlignment")] [SerializeField]
        TextToolsAlignment textAlignment = TextToolsAlignment.LeftTop;

        [FormerlySerializedAs("colliderType")] [SerializeField]
        TextToolsCollider colliderType = TextToolsCollider.None;

        [FormerlySerializedAs("lightProbes")] [SerializeField]
        LightProbeUsage lightProbes = LightProbeUsage.BlendProbes;

        [FormerlySerializedAs("castShadow")] [SerializeField]
        ShadowCastingMode castShadows = ShadowCastingMode.On;

        [FormerlySerializedAs("reflectText")] [SerializeField]
        bool reflectText = false;

        [FormerlySerializedAs("verticalText")] [SerializeField]
        bool verticalText = false;

        [FormerlySerializedAs("verticalPath")] [SerializeField]
        bool verticalPath = false;

        [FormerlySerializedAs("receiveShadows")] [SerializeField]
        bool receiveShadows = true;

        [FormerlySerializedAs("simulatePhysics")] [SerializeField]
        ...
        private void GenerateText(bool clear)
        {
            ...
            CenteringPivot(count);
            PlaceText();
        }

        private void CenteringPivot(int count)
        {
            GameObject go;
            var x = new Vector2(float.MaxValue, float.MinValue);
            var y = new Vector2(float.MaxValue, float.MinValue);
            for (int i = 0; i < count; ++i)
            {
                go = transform.GetChild(i).gameObject;
                var b = go.GetComponent<MeshRenderer>().bounds;
                if (b.min.x < x.x)
                {
                    x.x = b.min.x;
                }
            ...
            var size = new Vector3(x.y - x.x, y.y - y.x, 0);
            for (int i = 0; i < count; ++i)
            {
                go = transform.GetChild(i).gameObject;
